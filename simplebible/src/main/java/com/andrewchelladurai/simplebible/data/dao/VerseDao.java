...
public interface VerseDao {
    ...
    @Query(
        "select * from BIBLEVERSES where BOOKNUMBER = :book and CHAPTERNUMBER = :chapter order by VERSENUMBER ASC")
    LiveData<List<Verse>> getChapter(@IntRange(from = 1, to = 66) int book,
                                     @IntRange(from = 1) int chapter);

    @Query(
        "select * from BIBLEVERSES where BOOKNUMBER = :book and CHAPTERNUMBER = :chapter and VERSENUMBER = :verse order by VERSENUMBER ASC")
    LiveData<Verse> getVerse(@IntRange(from = 1, to = 66) int book,
                             @IntRange(from = 1) int chapter,
                             @IntRange(from = 1) int verse);

    @Query(
        "select * from BIBLEVERSES where VERSETEXT LIKE :text order by BOOKNUMBER, CHAPTERNUMBER , VERSENUMBER ASC")
